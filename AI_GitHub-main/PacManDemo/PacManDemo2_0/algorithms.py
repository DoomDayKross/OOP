...
class Algorithms:
    def __init__(self, food_positions):
        self.name = ""
        self.start_node = Node(PacMan(0, 0), [[]], 0, 0, [], 0)
        self.solution = []
        self.solution_ptr = 0
        self.explore_ptr = 0
        self.depth = 1
        ...
        while self.check_list:
            ...
            if not self.start_node.compare(node) and not self.is_in_explored(node):
                ...
                if node.is_goal:
                    ...
                    self.depth = node.depth
                    self.cost = node.cost
                    self.f = node.cost + node.heuristic_value
                return
            self.explored.append(node)
            node.expand()
            for i in range(len(node.nodes)):
