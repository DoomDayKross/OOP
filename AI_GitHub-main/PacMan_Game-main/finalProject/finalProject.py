...
def start_game():
    global start, moving, food_positions
    if moving == 1:
        moving = 0
    start = 1
    PacMan.state = 1
...
# Example usage for level1
grid = level0
food_positions_level0 = get_food_positions(level0)  
...
def choose_algorithm():
    if ai.sol_ptr == len(ai.solution):
        ...
        elif algorithm_text.get() == 'A-star Search':
            ai.a_star_search(start_node, food_positions_level0)  # Pass the goal as a list to find the nearest one
        return False
    else:
